...
export const Debug_SlaveManagement = observer(({beautifyValues}: {beautifyValues: boolean}) => {
  const {slaves} = useGame();
  return (
    <Debug>
      <Box display="flex" gap={2}>
        <Debug_ResourceGroup>
          <Debug_Resource resource={slaves.slaves_bound} beautifyValues={beautifyValues} incrementBy={1} decrementBy={1}/>
          ...
          <Stack direction="row" justifyContent="space-between">
            <Debug_Resource resource={slaves.slaves_roaming} beautifyValues={beautifyValues} incrementBy={1} decrementBy={1}/>
            <Button onClick={slaves.revive} sx={{width: 100}}>Revive</Button>
          </Stack>
          <Stack direction="row" justifyContent="space-between">
            <Debug_Resource resource={slaves.slaves_enslaved} beautifyValues={beautifyValues} incrementBy={1} decrementBy={1}/>
            <Button onClick={slaves.enslave} sx={{width: 100}}>Enslave</Button>
          </Stack>
          <Stack direction="row" justifyContent="space-between">
            <Debug_Resource resource={slaves.slaves_wasted} beautifyValues={beautifyValues} incrementBy={1} decrementBy={1}/>
            <Button onClick={slaves.waste} sx={{width: 100}}>Waste</Button>
          </Stack>
          <Stack direction="row" justifyContent="space-between">
            <Debug_Resource resource={slaves.slaves_consumed} beautifyValues={beautifyValues} incrementBy={1} decrementBy={1}/>
            <Button onClick={slaves.consume} sx={{width: 100}}>Consume</Button>
          </Stack>
        </Debug_ResourceGroup>

        <Debug_ResourceGroup>
          <Debug_Resource resource={slaves.slave_health} beautifyValues={beautifyValues} incrementBy={20} decrementBy={20}/>
