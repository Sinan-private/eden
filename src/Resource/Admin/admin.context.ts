...
import {ResourceKeys, ResourceState, ResourceTypes, ResourceUpdateProps} from "../types.ts";
...
const useAdminBase = () => {
  ...
  const write__initialResources = (newState?: ResourceState<ResourceKeys, ResourceTypes>[]) => {
    if (!newState) return;
